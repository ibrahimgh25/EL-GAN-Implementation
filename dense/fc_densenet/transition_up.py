...
class TransitionUp(RichRepr, Module):
    ...
    def __init__(self, upsample_channels: int, skip_channels: Optional[int] = None):
        ...
        self.out_channels = (
            upsample_channels + skip_channels if skip_channels is not None else None
        )
    ...
    def forward(self, upsample, skip):
        if self.skip_channels is not None and skip.shape[1] != self.skip_channels:
            raise ValueError(
                f"Number of channels in the skip connection input ({skip.shape[1]}) "
    ...
    def __repr__(self):
        ...
        return super(TransitionUp, self).__repr__(
            f"[{self.upsample_channels}, {skip_channels}] -> {out_channels})"
        )
