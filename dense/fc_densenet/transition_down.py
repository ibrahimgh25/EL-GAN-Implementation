...
class TransitionDown(RichRepr, Sequential):
    ...
    def __init__(
        self, in_channels: int, compression: float = 1.0, dropout: float = 0.0
    ):
        ...
        self.add_module(
            "conv", Conv2d(in_channels, self.out_channels, kernel_size=1, bias=False)
        )
    ...
    def __repr__(self):
        return super(TransitionDown, self).__repr__(
            self.in_channels, self.out_channels, dropout=self.dropout
        )
