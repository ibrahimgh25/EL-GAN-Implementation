...
disc_trainer = Trainer(
    disc, embedding_loss, SGD, sgd_params, ExponentialLR, scheduler_params
)
################################# End Region ###########################################
...
train_set = TuSimpleDataset(training_json, test_dir)
test_set = TuSimpleDataset(testing_json, train_dir)
...
NEEDED_SAMPLES_LOSS = (
    100  # Number of iterations before averaging and recording the loss
)
...
for epoch in range(EPOCHS):
    ...
    for i, data in enumerate(train_gen_loader):
        ...
        disc_loss = disc_trainer.backwards(
            real_embedding, fake_embedding, i % 500 >= 300
        )
        ...
        if not i % NEEDED_SAMPLES_LOSS and i:
            ...
            print(
                f"Training -- Epoch {epoch}, iteration {i}: gen_loss: {gen_avg}, disc_loss: {disc_avg}"
            )
    ...
    disc_trainer.save_model_img(
        f"models/discriminator/discriminator_epoch_{epoch}.ptmdl"
    )
    ...
    for i, data in enumerate(test_gen_loader):
        ...
        if not i % NEEDED_SAMPLES_LOSS and i:
            ...
            disc_avg = disc_running_loss / NEEDED_SAMPLES_LOSS
            print(
                f"Testing -- Epoch {epoch}, iteration {i}: gen_loss: {gen_avg}, disc_loss: {disc_avg}"
            )
            current_iter = epoch * len(train_gen_loader) + i
            writer.add_scalar("Generator Training Loss", gen_avg, current_iter)
            writer.add_scalar("Discriminator Training Loss", disc_avg, current_iter)
