...
sys.path.append("..")
from src import FCDenseNet
from src.densenet import FeatureBlock, Transition
...
class Discriminator(Sequential):
    def __init__(
        ...
        self.markings_head, num_channels_1 = self._separtate_part(
            1, initial_num_features, *dense_block_args
        )
        self.full_img_head, num_channels_2 = self._separtate_part(
            3, initial_num_features, *dense_block_args
        )
        current_channels = num_channels_1 + num_channels_2
        self.common_part, current_channels = self._common_part(
            current_channels, *dense_block_args
        )
    ...
    def _separtate_part(
        ...
        num_layers = [1, 2]
        model, current_channels = self.add_demse_blocks(
            model,
            name,
            num_layers,
            current_channels,
    ...
    def add_demse_blocks(
        ...
        dense_blocks_bottleneck_ratios = self.numeric_to_tuple(
            dense_blocks_bottleneck_ratios, num_blocks
        )
        transition_blocks_compression_factors = self.numeric_to_tuple(
            transition_blocks_compression_factors, num_blocks
        )
        dense_blocks_growth_rates = self.numeric_to_tuple(
            dense_blocks_growth_rates, num_blocks
        )
        ...
        dense_blocks_params = [
            ...
            for gr, nl, br in zip(
                dense_blocks_growth_rates, num_layers, dense_blocks_bottleneck_ratios
            )
        ]
    ...
    def apply(self, function):
        modules = [self.markings_head, self.full_img_head, self.common_part]
