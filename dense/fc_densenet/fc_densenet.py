...
class FCDenseNet(Module):
    ...
    def __init__(
        ...
        down_dense_bottleneck_ratios: Union[
        ...
        up_dense_bottleneck_ratios: Union[
            Optional[int], Sequence[Optional[int]]
        ] = None,
        ...
        if (
            down_dense_bottleneck_ratios is None
            or type(down_dense_bottleneck_ratios) == int
        ):
        ...
        if (
            up_dense_bottleneck_ratios is None
            or type(up_dense_bottleneck_ratios) == int
        ):
        ...
        self.features = Conv2d(
            in_channels, initial_num_features, kernel_size=5, padding=2, bias=False
        )
        ...
        down_dense_params = [
            ...
            for gr, nl, br in zip(
                down_dense_growth_rates,
                down_dense_num_layers,
                down_dense_bottleneck_ratios,
            )
        ]
        down_transition_params = [
            {"dropout": dropout, "compression": c}
        ...
        up_dense_params = [
            ...
            for gr, nl, br in zip(
                up_dense_growth_rates, up_dense_num_layers, up_dense_bottleneck_ratios
            )
        ]
    ...
    def forward(self, x):
        ...
        for skip, trans, dense in zip(
            reversed(skip_tensors), self.up_trans.children(), self.up_dense.children()
        ):
            res = trans(res, skip)
            res = dense(res)
